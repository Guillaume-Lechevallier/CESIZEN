...
auth_by_password_bp = Blueprint('admin_auth_by_password_bp', __name__)


@auth_by_password_bp.route('/admin/auth_by_password', methods=['GET'])
def auth_by_password():
    ...
    try:
        query = "SELECT id, password FROM admin_users WHERE email = %s"
        ...
        if user and bcrypt.checkpw(password.encode('utf-8'), user['password'].encode('utf-8')):
            ...
            insert_query = """
            INSERT INTO admin_authentification_token (id, suppression_date, token, type)
            VALUES (%s, %s, %s, %s)
            """
